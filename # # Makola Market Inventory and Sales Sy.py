...
transactions = 0  # Variable to count completed transactions
...
while True:
    ...
    if item not in inventory:
        ...
        if quantity > available_stock:
            print(f"Sorry, only {available_stock} units of {item} remaining.")
        else:
            total_cost = quantity * price
            inventory[item]["stock"] = available_stock - quantity
            transactions += 1
...
for speed in recorded_speeds:
    if speed > SPEED_LIMIT:
        print(
            "WARNING: Vehicle recorded at",
            speed,
            "km/h. Exceeded limit of",
            SPEED_LIMIT,
            "km/h."
        )
...
print("Percentage of vehicles speeding:", round(percentage_speeding, 2), "%")
print("Average speed of vehicles:", round(average_speed, 2), "km/h")

focused_segment = recorded_speeds[2:8]

print(
    "\nSpeeds for focused inspection segment (3rd to 8th vehicle):",
    focused_segment
)